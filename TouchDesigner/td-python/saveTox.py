...
class ExternalFiles:
    ...
    def __init__(self, my_op: OP) -> None:
        ...
        self._lister_COMP = my_op.op("widget_tox_finder/lister")
    ...
    def _colors_map(self) -> dict:
        ...
        colors_map = {
            "TouchDesigner": {
                "external_op": {"color": self.my_op.parGroup.Externalop, "tags": []}
            },
            "Python": {
                "default": {"color": self.my_op.parGroup.Pythondefault, "tags": []},
                "python_extension": {
    ...
    def Prompt_to_save(self) -> None:
        ...
        external_op_color = (
            self.Colors_map.get("TouchDesigner").get("external_op").get("color")
        )
    ...
    def Save_tox(self, current_loc, include_dir: bool = True):
        ...
        ext_color = self.Colors_map.get("TouchDesigner").get("external_op").get("color")
        ...
        current_loc.color = (
            self.op_none_color
            if current_loc.par["Sudotool"]
            else (ext_color[0], ext_color[1], ext_color[2])
        )
    ...
    def _save_tox(self, current_loc) -> None:
        ...
        # update hash_list
        self._ops_manager.Update_save_op_by_path(current_loc, False)

        # create and print log message
        log_msg = "{} saved to {}/{}".format(current_loc, project.folder, external_path)
...
""",
    ...
    def dialogChoice(self, info):
        ...
        if button_selection == "Cancel":
            ...
            # create and print log message
            log_msg = "{} saved to {}/{}".format(current_loc, project.folder, tox_path)
            self.Logtotextport(log_msg)
    ...
    def update_version_pars(self, target_op: OP, new_version: str = "") -> None:
        ...
        if target_op.par["Toxversion"] == None:
            self.update_custom_str_par(target_op, "Toxversion", "1.0.0", "Tox Version")
        else:
            if new_version == "":
        ...
        # updates tox version
        self.update_custom_str_par(target_op, "Toxversion", new_patch_version)
    ...
    def update_save_time(self, target_op: callable) -> None:
        ...
        if target_op.par["Lastsaved"] == None:
            self.update_custom_str_par(
                target_op, "Lastsaved", saveUtils.current_save_time(), "Last Saved"
            )
    ...
    def Set_ext_tox_colors(self):
        ...
        colors = self.Colors_map.get("TouchDesigner").get("external_op").get("color")
    ...
    def Set_external_file_colors(self):
        ...
        default_color = self.Colors_map.get("Python").get("default").get("color")
    ...
    def Keyboard_input(self, shortcut):
        ...
        self.my_op.par.Saveshortcut.eval()
        shortcut_lookup = {
            self.my_op.par.Saveshortcut.eval(): self.Prompt_to_save,
            self.my_op.par.Opensavewindowshortcut.eval(): self.Open_floating_external_tox_window,
        }
